<x-app-layout>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <form method="GET" action="{{ route('bukus.index') }}" style="display:flex;gap:8px">
                <div class="search-wrapper">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul, penulis..." class="search-input">
                </div>
                    ...
                    @forelse($bukus as $buku)
                        <tr>
                            ...
                            <td data-label="Judul Buku">
                                <div style="display:flex;align-items:center;gap:10px">
                                    <button type="button"
                                        ...
                                        data-fullsrc="{{ $buku->cover_image ? asset('storage/'.$buku->cover_image) : 'https://via.placeholder.com/300x420?text=Cover' }}"
                                        ...
                                        <img src="{{ $buku->cover_image ? asset('storage/'.$buku->cover_image) : 'https://via.placeholder.com/42x60?text=Cover' }}" alt="Cover {{ $buku->judul }}" style="width:100%;height:100%;object-fit:cover;">
                                    </button>
                                    <span style="font-weight:500">{{ $buku->judul }}</span>
                                </div>
                            </td>
                            <td data-label="Penulis" style="color:#64748b">{{ $buku->penulis }}</td>
                            <td data-label="Tahun"><span class="badge badge-secondary">{{ $buku->tahun_terbit }}</span></td>
                            <td data-label="Kategori">
                                @if($buku->kategori)
                                    <span class="badge badge-primary">{{ $buku->kategori->nama_kategori }}</span>
                                @else
                            ...
                            <td data-label="Stok">
                                ...
                                <span class="badge {{ $stok > 5 ? 'badge-success' : ($stok > 0 ? 'badge-warning' : 'badge-danger') }}">
                                    {{ $stok }} unit
                                </span>
                            </td>
                            <td data-label="Aksi">
                                <div style="display:flex;gap:6px">
                                    <a href="{{ route('bukus.show', $buku) }}" class="action-btn action-view" title="Lihat Detail">
                                        <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    <a href="{{ route('bukus.edit', $buku) }}" class="action-btn action-edit" title="Edit">
                                        <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form method="POST" action="{{ route('bukus.destroy', $buku) }}" style="margin:0">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="action-btn action-delete" title="Hapus" onclick="event.preventDefault(); showDeleteModal(this.form, '{{ $buku->judul }}')">
