<x-app-layout>
    ...
    <div class="peminjaman-summary-grid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24px;">
        ...
        <div class="stat-card">
            ...
            <div>
                ...
                <div class="stat-value">{{ $totalRak }}</div>
            </div>
        </div>
        <div class="stat-card">
            ...
            <div>
                ...
                <div class="stat-value">{{ $totalBelumDitempatkan }}</div>
            </div>
        </div>
    </div>

    {{-- Rak Sections --}}
    @forelse($bukuByRak as $rakNomor => $bukus)
        <div class="card" style="margin-bottom:20px;">
            <div class="card-header" style="background:linear-gradient(135deg,rgba(99,102,241,0.06),rgba(139,92,246,0.04));">
                <h3 style="display:flex;align-items:center;gap:10px;">
                    ...
                    <div>
                        <div style="font-size:16px;font-weight:700;">Rak {{ $rakNomor }}</div>
                        <div style="font-size:12px;color:#64748b;font-weight:400;">{{ $bukus->count() }} buku</div>
                    </div>
                </h3>
            </div>
                                ...
                                <td data-label="Judul">
                                    <div style="display:flex;align-items:center;gap:10px">
                                        <div style="width:38px;height:52px;border-radius:8px;overflow:hidden;border:1px solid #e2e8f0;background:#f8fafc;flex-shrink:0;">
                                            <img src="{{ $buku->cover_image ? asset('storage/'.$buku->cover_image) : 'https://via.placeholder.com/38x52?text=📖' }}" alt="" style="width:100%;height:100%;object-fit:cover;">
                                        </div>
                                        <span style="font-weight:500">{{ $buku->judul }}</span>
                                    </div>
                                </td>
                                <td data-label="Penulis" style="color:#64748b;font-size:14px;">{{ $buku->penulis }}</td>
    ...
    {{-- Buku Belum Ditempatkan --}}
    @if($bukuTanpaRak->count() > 0)
        <div class="card" style="margin-bottom:20px;">
            <div class="card-header" style="background:linear-gradient(135deg,rgba(245,158,11,0.08),rgba(217,119,6,0.04));">
                <h3 style="display:flex;align-items:center;gap:10px;">
                    ...
                    <div>
                        ...
                        <div style="font-size:12px;color:#64748b;font-weight:400;">{{ $bukuTanpaRak->count() }} buku belum memiliki nomor rak</div>
                    </div>
                </h3>
            </div>
            <div class="table-wrapper">
                    ...
                    <tbody>
                        @foreach($bukuTanpaRak as $buku)
                            <tr>
                                ...
                                <td data-label="Aksi">
                                    <a href="{{ route('bukus.edit', $buku) }}" class="btn btn-warning btn-sm" style="font-size:12px;">
    ...
    @if($bukuByRak->isEmpty() && $bukuTanpaRak->isEmpty())
