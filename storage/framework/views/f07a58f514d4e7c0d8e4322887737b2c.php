        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px">
            ...
            <div style="background:linear-gradient(135deg,#eef2ff,#e0e7ff);border:1px solid #c7d2fe;border-radius:14px;padding:16px 20px;display:flex;align-items:center;gap:12px">
                ...
                <div>
                    ...
                    <p style="font-size:13px;color:#6366f1;margin:2px 0 0"><?php echo e(Auth::user()->name); ?></p>
                </div>
            </div>

            
            <?php $dendaPerHari = \App\Models\Setting::get('denda_per_hari', 5000); ?>
            <div style="background:linear-gradient(135deg,#fffbeb,#fef3c7);border:1px solid #fde68a;border-radius:14px;padding:16px 20px;display:flex;align-items:center;gap:12px">
                ...
                <div>
                    ...
                    <p style="font-size:13px;color:#a16207;margin:2px 0 0">Rp <?php echo e(number_format($dendaPerHari, 0, ',', '.')); ?>/hari/buku jika terlambat</p>
                </div>
            </div>
        </div>

        <form method="POST" action="<?php echo e(route('peminjamans.store')); ?>" id="formPeminjaman">
            <?php echo csrf_field(); ?>
                        ...
                        <select name="siswa_id" id="siswa_id" class="form-control" required>
                            ...
                            <?php $__currentLoopData = $siswas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $siswa): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($siswa->id); ?>" <?php echo e(old('siswa_id') == $siswa->id ? 'selected' : ''); ?>>
                                    <?php echo e($siswa->nama); ?> (<?php echo e($siswa->nis); ?>) — <?php echo e($siswa->kelas); ?> <?php echo e($siswa->jurusan); ?>
...
unset($__errorArgs, $__bag); ?>
            ...
            <div class="card" style="margin-bottom:20px">
                ...
                <div class="card-body">
                    <?php $__errorArgs = ['buku_ids'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                        <p style="color:#ef4444;font-size:13px;margin-bottom:12px"><?php echo e($message); ?></p>
                    <?php unset($message);
if (isset($__messageOriginal)) { $message = $__messageOriginal; }
endif;
unset($__errorArgs, $__bag); ?>
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(220px, 1fr));gap:12px" id="bookGrid">
                        <?php $__empty_1 = true; $__currentLoopData = $bukus; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $buku): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <label class="book-card" data-judul="<?php echo e(strtolower($buku->judul)); ?>" data-penulis="<?php echo e(strtolower($buku->penulis)); ?>" style="display:flex;align-items:flex-start;padding:14px;border:2px solid #e2e8f0;border-radius:14px;cursor:<?php echo e($buku->stok <= 0 ? 'not-allowed' : 'pointer'); ?>;transition:all 0.2s;background:<?php echo e($buku->stok <= 0 ? '#f8fafc' : 'white'); ?>;opacity:<?php echo e($buku->stok <= 0 ? '0.6' : '1'); ?>" onmouseover="if(<?php echo e($buku->stok); ?>>0){this.style.borderColor='#a5b4fc';this.style.background='#eef2ff';this.style.transform='translateY(-2px)';this.style.boxShadow='0 4px 12px rgba(99,102,241,0.1)'}" onmouseout="if(<?php echo e($buku->stok); ?>>0){this.style.borderColor=this.querySelector('input').checked?'#6366f1':'#e2e8f0';this.style.background=this.querySelector('input').checked?'#eef2ff':'white';this.style.transform='none';this.style.boxShadow='none'}">
                                <input type="checkbox" name="buku_ids[]" value="<?php echo e($buku->id); ?>" 
                                       style="margin-right:12px;margin-top:2px;cursor:pointer;width:18px;height:18px;accent-color:#6366f1"
                                       <?php echo e(collect(old('buku_ids', []))->contains($buku->id) ? 'checked' : ''); ?>

                                       <?php echo e($buku->stok <= 0 ? 'disabled' : ''); ?>

                                       onchange="updateBookSelection()">
                                <div style="flex:1">
                                    <div style="font-weight:600;color:#0f172a;font-size:14px;margin-bottom:6px;line-height:1.3"><?php echo e($buku->judul); ?></div>
                                    <div style="font-size:12px;color:#64748b;margin-bottom:6px"><?php echo e($buku->penulis); ?></div>
                                    <div>
                                        <?php if($buku->stok > 0): ?>
                                            <span style="background:#dcfce7;color:#166534;padding:3px 8px;border-radius:6px;font-size:11px;font-weight:600">✓ Stok: <?php echo e($buku->stok); ?></span>
                                        <?php else: ?>
                ...
                <div class="card-body">
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
                        <div class="form-group">
                            ...
                            <input type="date" name="tanggal_pinjam" id="tanggal_pinjam" value="<?php echo e(old('tanggal_pinjam', date('Y-m-d'))); ?>" class="form-control" required>
...
unset($__errorArgs, $__bag); ?>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="batas_kembali" id="batas_kembali" value="<?php echo e(old('batas_kembali')); ?>" class="form-control" required>
                            <p style="font-size:12px;color:#94a3b8;margin:6px 0 0">Denda Rp <?php echo e(number_format($dendaPerHari, 0, ',', '.')); ?>/hari jika melewati batas</p>
                            <?php $__errorArgs = ['batas_kembali'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div class="form-group" style="margin-bottom:0">
                        ...
                        <select name="status" id="status" class="form-control" required>
                            <option value="dipinjam" <?php echo e(old('status', 'dipinjam') == 'dipinjam' ? 'selected' : ''); ?>>📤 Dipinjam</option>
...
unset($__errorArgs, $__bag); ?>
            ...
            <div style="display:flex;gap:12px;justify-content:space-between;align-items:center">
                ...
                <div style="display:flex;gap:10px">
                    <a href="<?php echo e(route('peminjamans.index')); ?>" class="btn btn-secondary">
...
<?php endif; ?><?php /**PATH C:\laragon\www\Azzam-sp-XI-PPLG-1-TUGAS-MIGRATION\resources\views/peminjamans/create.blade.php ENDPATH**/ ?>